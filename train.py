...
def train(epoch):
    ...
    for batch in tqdm(training_data_loader):
        ...
        loss_rgb = L1_loss(output_rgb, gt_rgb) + D_loss(output_rgb, gt_rgb) + E_loss(output_rgb, gt_rgb) + opt.P_weight * P_loss(output_rgb, gt_rgb)[0] + opt.ddsrn_weight * DDSRN_loss(output_rgb, gt_rgb)
        ...
        if iter == train_len:
            ...
            if not os.path.exists(opt.val_folder+'training_switch1e-2_ACM'):          
                os.mkdir(opt.val_folder+'training_switch1e-2_ACM') 
            output_img.save(opt.val_folder+'training_switch1e-2_ACM/test.png')
            gt_img.save(opt.val_folder+'training_switch1e-2_ACM/gt.png')
...
def checkpoint(epoch):
    if not os.path.exists("./weights"):          
        os.mkdir("./weights") 
    if not os.path.exists("./weights/train_switch1e-2_ddsrnACM"):          
        os.mkdir("./weights/train_switch1e-2_ddsrnACM")  
    model_out_path = "./weights/train_switch1e-2_ddsrnACM/epoch_{}.pth".format(epoch)
...
def load_datasets():
    ...
    elif opt.dataset == 'LOLI':
        train_set = get_LOLI_training_set(opt.data_train_LOLI,size=opt.cropSize)
        test_set = get_eval_set(opt.data_val_LOLI)
    else:
...
def init_loss():
    ...
    DDSRN_loss = DDSRN_Loss(model_path='./weights/DDSRN_ACM/best_model.pt', device='cuda')
    return L1_loss,P_loss,E_loss,D_loss, DDSRN_loss
...
if __name__ == '__main__':  
    ...
    L1_loss,P_loss,E_loss,D_loss, DDSRN_loss = init_loss()
    ...
    for epoch in range(start_epoch+1, opt.nEpochs + start_epoch + 1):
        ...
        if epoch % opt.snapshots == 0:
            ...
            if opt.dataset == 'LOLI':
                output_folder = 'LOLI/'
                label_dir = opt.data_valgt_LOLI  # Make sure this path exists in your options
                norm_size = True  # LoLI is paired/sized correctly, so True is safe
    ...
    with open(f"./results/training_switch1e-2_ACM/metrics{now}.md", "w") as f:
        ...
        f.write(f"DDSRN_weight: {opt.ddsrn_weight}\n")
