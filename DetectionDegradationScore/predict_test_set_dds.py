...
def benchmark_dds_calculation(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(test_loader, desc="Processing batches")):
            ...
            if batch_idx == warmup_batches:
                ...
                pre_detection_memory = measure_memory_variations(
                    device_id, "pre-detection"
                )
            ...
            if is_benchmark_batch and batch_idx == warmup_batches:
                post_detection_memory = measure_memory_variations(
                    device_id, "post-detection"
                )
            ...
            if is_benchmark_batch and batch_idx == warmup_batches:
                post_matching_memory = measure_memory_variations(
                    device_id, "post-matching"
                )
    ...
    avg_detection_time_per_image = (
        total_detection_time / total_images if total_images > 0 else 0
    )
    avg_matching_time_per_image = (
        total_matching_time / total_images if total_images > 0 else 0
    )
    avg_total_time_per_image = total_time / total_images if total_images > 0 else 0
    detection_fps = (
        total_images / total_detection_time if total_detection_time > 0 else 0
    )
    ...
    output_data = {
        ...
        "dds_scores": results[
            :100
        ],  # Just include the first 100 results for verification
    }
    ...
    print(
        f"Average detector time per image: {avg_detection_time_per_image * 1000:.4f} ms"
    )
    print(
        f"Average matching time per image: {avg_matching_time_per_image * 1000:.4f} ms"
    )
