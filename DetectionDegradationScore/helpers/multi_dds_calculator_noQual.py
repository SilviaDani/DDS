...
# Dataset class for GT/Distorted pairs
...
# Dataloader loader
def create_dataloaders(dataset_root, batch_size):
...
class ErrorScoreCalculator:
    ...
    def process_batch(self, gt_images: torch.Tensor, sr_images: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            preds = self.yolo_model.predict(sr_images, verbose=False)
            matches = match_predictions(gt_preds, preds)
