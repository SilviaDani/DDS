...
class BaseModel():
    ...
    def load_network(self, net, name, tag='net_sr'):
        ...
        # If it's a wrapped checkpoint dict (like in the full checkpoint), unwrap it.
        if isinstance(load_net, dict) and 'params' in load_net:
            load_net = load_net.get('params', load_net)

        self.text_logger.write(f'--> Loading {tag} model from {load_path}')

        # Remove 'module.' prefix from DDP models if present.
        load_net = {k[7:] if k.startswith('module.') else k: v for k, v in load_net.items()}
