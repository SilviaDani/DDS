...
class SR4IRDetectionModel(BaseModel):
    ...
    def __init__(self, opt):
        super().__init__(opt)
        wandb.init(
            project=opt.get('wandb_project', 'SR4IR-Detection'),
            name=opt.get('wandb_run_name', None),  # Use this variable for run name
            config=opt
        )
    ...
    def train_one_epoch(self, data_loader_train, train_sampler, epoch):
        ...
        for iter, (img_hr_list, target_list) in enumerate(metric_logger.log_every(data_loader_train, self.opt['print_freq'], self.text_logger, header)):
            ...
            #print("target_list:", target_list)  # Debugging line to print target_list
            ...
            if hasattr(self, 'cri_pix'):
                ...
                wandb.log({'losses/l_pix': l_pix.item(), 'iter': current_iter, 'epoch': epoch})
                l_total_sr += l_pix
            if epoch > self.warmup_epoch:
                if hasattr(self, 'cri_tdp'):
                    self.net_det.eval()
                    _, _, feat_sr = self.net_det(img_sr_list, return_feats=True) #Keys in feat_sr['features']: ['0', '1', 'pool']
                    ...
                    wandb.log({'losses/l_tdp': l_tdp.item(), 'iter': current_iter, 'epoch': epoch})
                    l_total_sr += l_tdp
            l_total_sr.backward()
            ...
            _, _, total_feat_sr = self.net_det(img_sr_list, return_feats=True) #Keys in feat_sr['features']: ['0', '1', 'pool']
            ...
            if hasattr(self, 'cri_det_cqmix'):
                ...
                _, _, total_feat_cqmix = self.net_det(img_cqmix_list, return_feats=True)
                feat_cqmix = {k: total_feat_cqmix['features'][k] for k in list(total_feat_cqmix['features'].keys())[:2]}
                l_det_cqmix = self.cri_det_hr(feat_hr, feat_cqmix)
                ...
                wandb.log({'losses/l_det_cqmix': l_det_cqmix.item(), 'iter': current_iter, 'epoch': epoch})
            ...
            wandb.log({'psnr': psnr.item(), 'iter': current_iter, 'epoch': epoch})
    ...
    def evaluate(self, data_loader_test, epoch=0):
        ...
        # --- wandb logging for AP and recall ---
        for iou_type, coco_eval in coco_evaluator.coco_eval.items():
            ...
            wandb.log({
                ...
                f'{iou_type}/AR1': stats[6],
                f'{iou_type}/AR10': stats[7],
                f'{iou_type}/AR100': stats[8],
                f'{iou_type}/ARs': stats[9],
                f'{iou_type}/ARm': stats[10],
                f'{iou_type}/ARl': stats[11],
                'epoch': epoch
            })
        return
