...
class SR4IRDetectionModel(BaseModel):
    def __init__(self, opt):
        ...
        self.net_sr = self.model_to_device(self.net_sr, is_trainable=False)
        self.net_sr.eval()
        
        # 2. YOLOv8x
        yolo_path = opt.get('network_det', {}).get('weights_yolo', 'yolo26x.pt') 
        self.yolo_model = YOLO(yolo_path)
        self.yolo_model.to(self.device)

        # 3. RT-DETR
        rtdetr_path = opt.get('network_det', {}).get('weights_rtdetr', 'rtdetr-l.pt') 
        ...
        self.vis_dir = osp.join(self.opt.get('path', {}).get('experiments_root', './experiments'), 'det/034_DDSFeat_SR4IR_swinir_x4_30epochs_3warmup_lambda1_ODVirat')
