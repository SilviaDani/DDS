...
class FasterRCNN(GeneralizedRCNN):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(backbone, rpn, roi_heads, transform, return_feats=return_feats)
...
def _fasterrcnn_mobilenet_v3_large_fpn(
    ...
    **kwargs: Any,
    ...
    # Convert the Enum object back to a string because get_weight expects a string to split
    if weights_backbone is not None:
        # This turns it back into "MobileNet_V3_Large_Weights.IMAGENET1K_V1"
        wb_str = f"MobileNet_V3_Large_Weights.{weights_backbone.name}"
    else:
        wb_str = None
    backbone = mobilenet_v3_large(weights=wb_str, progress=progress, norm_layer=norm_layer)
    backbone = _mobilenet_extractor(backbone, True, trainable_backbone_layers)
    
    # --- ADD THIS BLOCK ---
    # Standard FPNs in Torchvision always output 256 channels.
    if not hasattr(backbone, "out_channels"):
